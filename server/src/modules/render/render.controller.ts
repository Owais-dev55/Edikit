...
export class RenderController {
  ...
  @Public()
  @ApiOperation({
    summary: 'Get template field metadata with accurate dimensions',
  })
  @ApiResponse({
    status: 200,
    description: 'Template field metadata retrieved successfully',
  })
  async getTemplateFields(
    @Param('templateId', ParseIntPipe) templateId: number,
  ) {
    return await this.renderService.getTemplateFields(templateId);
  }

  @Put('templates/:templateId/fields')
  @ApiOperation({
    summary: 'Update template field dimensions (admin only)',
  })
  @ApiCookieAuth()
  @ApiBody({
    schema: {
  ...
  @ApiResponse({
    status: 200,
    description: 'Template fields updated successfully',
  })
  async updateTemplateFields(
    @Param('templateId', ParseIntPipe) templateId: number,
    @Body() body: { fields: Record<string, { width: number; height: number; layerName?: string }> },
  ) {
    return await this.renderService.updateTemplateFields(templateId, body.fields);
  }}