...
export class RenderService {
  ...
  /**
   * Get template field metadata with dimensions
   */
  async getTemplateFields(templateId: number) {
    ...
    // Get field dimensions from layers JSON field
    const layersData = (template.layers as any) || {};
    const fieldDimensions = layersData.fieldDimensions || {};

    // Get predefined layer mapping to know which fields exist
    const layerMapping = this.getPredefinedLayerMapping(templateId) || {};
    ...
    // Build field metadata from layer mapping
    Object.entries(layerMapping).forEach(([fieldKey, layerName]) => {
      fields[fieldKey] = {
        layerName: layerName as string,
        ...(fieldDimensions[fieldKey] || {}),
      };

      // Generate dimension string if width/height are available
      if (fieldDimensions[fieldKey]?.width && fieldDimensions[fieldKey]?.height) {
        fields[fieldKey].dimensions = `${fieldDimensions[fieldKey].width}x${fieldDimensions[fieldKey].height}`;
      }
    });
  ...
  /**
   * Update template field dimensions (admin only)
   */
  async updateTemplateFields(
    templateId: number,
    fields: Record<string, { width: number; height: number; layerName?: string }>,
  ) {
    const template = await this.prisma.nexrenderTemplate.findUnique({
      where: { templateId },
    });
    ...
    // Get existing layers data or create new
    const layersData = (template.layers as any) || {};
    
    // Store field dimensions in layers.fieldDimensions
    layersData.fieldDimensions = layersData.fieldDimensions || {};
    
    Object.entries(fields).forEach(([fieldKey, metadata]) => {
      layersData.fieldDimensions[fieldKey] = {
        width: metadata.width,
        height: metadata.height,
        ...(metadata.layerName && { layerName: metadata.layerName }),
      };
    });
    ...
    this.logger.log(`Updated field dimensions for template ${templateId}`);

    return {
      templateId,
      message: 'Field dimensions updated successfully',
      fields: layersData.fieldDimensions,
    };
  }}